...
export default async function transpile(options: Partial<TranspilerOptions> = {}) {
    ...
    output = `imgui_disable_vector_packing=true${luaverConfig.lineSeparator}PLUGIN_NAME="${luaverConfig.pluginName}";PLUGIN_VERSION="${luaverConfig.pluginVersion}";PLUGIN_AUTHOR="${luaverConfig.pluginAuthor}";PLUGIN_DESCRIPTION="${luaverConfig.pluginDescription}"${luaverConfig.lineSeparator}ENVIRONMENT="${options.environment ?? 'development'}";DISTRO="${options.distro ?? 'development'}"${luaverConfig.lineSeparator}${output}`;
    if (!luaverConfig.dontRandomizeSeed) output = `math.randomseed(os.time())${luaverConfig.lineSeparator}${output}`;

    output = `-- [${luaverConfig.pluginName} ${luaverConfig.pluginVersion}] (Compiled by Luaver)${luaverConfig.lineSeparator}-- DO NOT EDIT THIS FILE DIRECTLY.${luaverConfig.lineSeparator.repeat(2)}${output}`;
